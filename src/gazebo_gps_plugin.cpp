                          ...
                          ignition::gazebo::EventManager &_em)
{
  ...
  if (_sdf->HasElement("gpsTopic"))
  {
    gazebo::getSdfParam<std::string>(_sdf, "gpsTopic", gps_topic_, kDefaultGpsTopic);
  }
  else
  {
                           ...
                           const ignition::gazebo::EntityComponentManager &_ecm)
{
  ...
  {
    const ignition::math::Pose3d T_W_I = _ecm.ComponentData<ignition::gazebo::components::WorldPose>(model_link_).value();
    ...
    const ignition::math::Vector3d velocity_current_W = _ecm.ComponentData<ignition::gazebo::components::WorldLinearVelocity>(model_link_).value();
